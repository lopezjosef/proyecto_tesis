# -*- coding: utf-8 -*-
"""
Created on Thu Dec 29 00:56:27 2022
...
for agrupacion in agrupaciones:
    ...
    for train_index, test_index in kfold.split(X_train):
        ...
        model = Sequential()
        model.add(Dense(128,input_dim=189,activation='linear'))
    ...
    mae_val_loss_cv = np.array(resultados_cv_val_loss)[:,0].mean()
    ...
    plot_prediccion.figure.savefig('graficos_cv/plot_prediccion_'+agrupacion.replace('/','').replace(' ','')+'.png', dpi=150)
    
    entrenamiento = pd.DataFrame({'loss':historia.history['loss']})
    entrenamiento_plot = entrenamiento.plot()
    entrenamiento_plot.figure.savefig('graficos_cv/entrenamiento_'+agrupacion.replace('/','').replace(' ','')+'.png', dpi=150)
    ...
    dif_plot_density.figure.savefig('graficos_cv/densidad_errores_'+agrupacion.replace('/','').replace(' ','')+'.png', dpi=150)
    ...
    model.save('modelos_cv/'+'model_'+agrupacion.replace('/','').replace(' ','')+'.h5') 
    
    print('Entrenamiento completado:'+agrupacion)
    

resultados.to_excel('resultados_entrenamiento_cv.xlsx',index=False)
